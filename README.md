...
| 節 | 内容 | 使っている 3D |
| --- | --- | --- |
| 開幕 | 初回だけ出る幕 | 紋が裏から横を経て正面を向く 36 コマ |
...
| （お品書きの奥） | スクロールで回る背景 | 開幕と同じ紋の連番を、スクロール量で送る |
| ご案内 | 営業時間・アクセス | 紋（黒漆に金の「雅」）・箸 |

## つくりで気をつけたところ

- **開幕は 3D の連番**。ページを開くと、紋が裏から正面へ回って店名が出る。
  セッション中は一度きり（`sessionStorage`）で、クリックやスクロールで飛ばせる。
  JS が無い環境では幕そのものが出ず、読み込み時間も伸びない。
  何かで止まっても必ず開くよう、`<head>` 側に時限の保険を置いてある。
- **スクロールで紋が回る**。お品書きの奥に置いた紋は、その節を通り過ぎる量を
  そのままコマ番号に写している。ヒーローの写真もわずかに視差で動く。
  どちらも `prefers-reduced-motion` では止まる。
